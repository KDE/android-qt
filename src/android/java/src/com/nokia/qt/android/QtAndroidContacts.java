...
public class QtAndroidContacts
{
	...
	private OnlineAccount[] getImDetails(String id)
	{
		String imWhere = ContactsContract.Data.CONTACT_ID + " = ? AND " + ContactsContract.Data.MIMETYPE + " = ?"; 
     	...
     	Cursor imCur = _m_instance.getContentResolver().query(ContactsContract.Data.CONTENT_URI, null, imWhere, imWhereParams, null);
     	if(imCur!=null)
     	{
	...
	{
		String id=null;
		Cursor allContactsCur =  _m_instance.managedQuery(RawContacts.CONTENT_URI, new String[] {RawContacts._ID}, RawContacts.CONTACT_ID + " = " + rid, null, null); 
	...
	private String saveContact(MyContacts qtContacts)
	{
		...
		Uri rawContactUri = _m_instance.getContentResolver().insert(RawContacts.CONTENT_URI, values);
	...
	{
		int deleted = 0;
		ContentResolver cr = _m_instance.getContentResolver();
		String where = ContactsContract.Data.CONTACT_ID + " = ?"; 
		String[] whereParams = new String[]{id}; 
		Cursor rmCur = _m_instance.getContentResolver().query(ContactsContract.Data.CONTENT_URI, null, where, whereParams, null);
	...
	private void updateNameDetails(NameData names,String id)
	{
		...
		if(nameCur.moveToFirst())
		{
			...
			_m_instance.getContentResolver().delete(ContentUris.withAppendedId(ContactsContract.Data.CONTENT_URI,Long.parseLong(nid)),null,null);
		}
...
class MyContacts
{
	...
	public MyContacts(NameData names,PhoneNumber[] phoneNumberArray,EmailData[] emailArray,String note,AddressData[] addressArray,OrganizationalData[] organizationArray,String birthday,String anniversary,String nickName,String[] urlArray)
	{
		...
		Log.i("OrgCount",Integer.toString(orgcount));
			...
			String anniversary,String nickName,String[] urlArray,OnlineAccount[] onlineAccountArray)
	{
		...
		else
		{
			...
			m_onlineAccount[0] = new OnlineAccount(new String(), new String(),0,0, 0, 0);
		}
...
class OnlineAccount
{
	...
	public OnlineAccount(String account,String status,long timeStamp,String customProtocol,int presence,int protocol,int type)
	{
		m_account = new String(account);
		m_status = new String(status);
		m_customProtocol = new String(customProtocol);
		m_timeStamp = timeStamp;
		m_presence = presence;
		m_protocol = protocol;
		m_type = type;
