...
void qt_checkAndWarnAboutGLError(const char* functionName, const char* glFunctionName)
{
    GLenum glerror = glGetError();
    if(glerror != GL_NO_ERROR)
        qWarning("An error in Android-EGLFS-QPA-plugin occurred in funtion %s, when calling: %s. glGetError returned: %x         Be aware that this could also be caused by a previous (unchecked) gl-function." ,
                 functionName, glFunctionName, glerror);

}
void qt_checkAndWarnAboutEGLError(const char* functionName, const char* eglFunctionName)
{
    GLenum eglerror = eglGetError();
    if(eglerror != EGL_SUCCESS)
        qWarning("An error in Android-EGLFS-QPA-plugin occurred in funtion %s, when calling %s. eglGetError returned: %x", functionName, eglFunctionName, eglerror);
}
